...
describe("JobDetailDrawer", () => {
  ...
  it("renders job details and the recommended hint", () => {
    ...
    expect(screen.getAllByText("Ruby on Rails").length).toBeGreaterThanOrEqual(1);
    expect(screen.getAllByText("TypeScript").length).toBeGreaterThanOrEqual(1);
    expect(screen.getByText("東京")).toBeInTheDocument();
    expect(screen.getByText("比較レーダー")).toBeInTheDocument();
    expect(screen.getByText("応募判断")).toBeInTheDocument();
  });
